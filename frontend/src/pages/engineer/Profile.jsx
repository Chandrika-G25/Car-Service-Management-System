...
import { Save, Lock, Wrench, Shield, CheckCircle } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';
import { authService } from '../../services/authService';

export const EngineerProfile = () => {
  const { user, updateUser } = useAuth();
  const prof = user?.engineer_profile || {};
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      updateUser(res.user);
      setProfileMsg({ type: 'success', text: 'Technician status and profile updated.' });
    } catch (err) {
    ...
    <div style={{ maxWidth: '850px' }}>
      <div className="page-header">
        <div className="page-title">
          <h1>Technician Profile & Shift Status</h1>
          <p>
            Employee #{prof.employee_id || 'ENG'} • Specialization: {prof.specialization || 'Diagnostic Tech'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '2rem' }}>
        <div className="card">
          <h3 style={{ marginBottom: '1.25rem' }}>Availability & Details</h3>
          {profileMsg.text && (
          ...
          <form onSubmit={handleProfileSubmit}>
            ...
            <div className="form-group">
              <label className="form-label">Bay Shift Availability Status *</label>
              <select
                className="form-select"
                value={formData.availability_status}
                onChange={(e) => setFormData({ ...formData, availability_status: e.target.value })}
              >
                <option value="AVAILABLE">Available (Ready for workorder dispatch)</option>
                <option value="BUSY">Busy (Active on complex bay diagnostics)</option>
        ...
        <div className="card">
          <h3 style={{ marginBottom: '1.25rem' }}>Security & Access</h3>
          {passMsg.text && (
